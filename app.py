...
if menu == "Option A: DB Management":
    ...
    with tab1:
        ...
        with header_col2:
            ...
            if st.button("🔄 DB 새로고침", help="최신 데이터베이스 정보를 다시 불러옵니다.", use_container_width=True, key="refresh_db_button1"):
                st.rerun()
    ...
    with tab2:
        ...
        with col1:
            ...
            # 통합 업데이트 버튼
            if st.button("Update Historical", use_container_width=True, disabled=invalid_date):
        ...
        with col2:
            ...
            # 통합 업데이트 버튼
            if st.button("Update Forecast (All)", use_container_width=True, disabled=invalid_fore_date):
    ...
    with tab3:
        ...
        with header_col2:
            # st.rerun()을 사용하여 F5 대신 Streamlit 앱만 새로고침 되도록 처리
            if st.button("🔄 DB 새로고침", help="최신 데이터베이스 정보를 다시 불러옵니다.", use_container_width=True):
                st.rerun()
...
# --- Option D: Visualization ---
    ...
    if df_res.empty or 'est_Solar_Utilization' not in df_res.columns or df_res['est_Solar_Utilization'].isnull().all():
        ...
        if selected_vars:
            ...
            st.dataframe(df[display_cols].style.apply(highlight_warnings, axis=1), use_container_width=True)
...
# --- Option E: Validation ---
    ...
    with header_col2:
        if st.button("⚡ 실시간 데이터 가져오기", use_container_width=True, type="primary"):
